	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	// anything in GPDB.
	if (m_cmd_type == CMD_UPDATE && phy_dml_dxlop_dmltype == gpdxl::Edxldmlsplitupdate)
	{
	...
	if (phy_dml_dxlop_dmltype == gpdxl::Edxldmlsplitupdate && (m_cmd_type == CMD_UPDATE && phy_dml_dxlop->IsOidsPreserved()))
	{
		AddJunkTargetEntryForColId(&dml_target_list, &child_context,
	...
	{
		dml->isSplitUpdates = (phy_dml_dxlop_dmltype == gpdxl::Edxldmlsplitupdate)? ListMake1Int((int) true): ListMake1Int((int) false);
	}
